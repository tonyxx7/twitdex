...
Twitter Library 2.0
*/
package actionscript.twitterconnection
{
	 ...
	 */ 
	public class TwitterConnector extends EventDispatcher
	{
		// for search
...
private static const FAVORITES:String = "favorites/";
private static const CREATE_FAVORITE:String = "createFavorite";
private static const DESTROY_FAVORITE:String = "destroyFavorite";

private static const CREATE_LISTS:String = "List";
private static const UPDATE_LISTS:String = "List";
private static const GET_LISTS:String = "List";
/**
private static const _LIST:String = "List";
private static const _LIST:String = "List";
private static const _LIST:String = "List";
private static const _LIST:String = "List";
private static const _LIST:String = "List";
...
/**/
		...
		private static const LOAD_FRIENDS_TIMELINE_URL_SINCE:String = 
			"http://twitter.com/statuses/friends_timeline/$userId.xml?" + 
			"since_id=$since_id";
...
private static const FAVORITES_URL:String = 
	"http://twitter.com/favorites/${id}.xml";
private static const CREATE_FAVORITE_URL:String = 
	"http://twitter.com/favorites/create/${id}.xml";
private static const DESTROY_FAVORITE_URL:String = 
...
this.addLoader(FAVORITES, favoritesHandler);
this.addLoader(CREATE_FAVORITE, favoriteCreatedHandler);
this.addLoader(DESTROY_FAVORITE, favoriteDestroyedHandler);
...
/**/
		...
		*/
		public function loadFriendsTimeline(userId:String,count:int=20,sinceId:int=0):void
		{
			...
			friendsTimelineLoader.load(twitterRequest(LOAD_FRIENDS_TIMELINE_URL.replace("$userId",userId)
				.replace("$count",String(count)).replace("$since_id",String(sinceId))));
		}
		
		/**
		* Loads the timeline of all friends on Twitter. Authentication required for private users.
		*/
		public function loadFriendsTimelineSince(userId:String,sinceId:int=0):void
		{
			var friendsTimelineLoader:URLLoader = this.getLoader(FRIENDS_TIMELINE);
			friendsTimelineLoader.load(twitterRequest(LOAD_FRIENDS_TIMELINE_URL_SINCE.replace("$userId",userId)
				.replace("$since_id",String(sinceId))));
		}
		...
		*/
		public function loadUserTimeline(userId:String,count:int=20,page:int=1):void
		{
			...
			userTimelineLoader.load(twitterRequest(
				LOAD_USER_TIMELINE_URL.replace("$userId", userId)
				.replace("$count",String(count)).replace("$page",String(page))
			));
		}
...
public function search(query:TwitterSearch):void
{
	...
	var url:String = SEARCH_URL + query;
	/*
	...
	*/
	searchLoader.load(twitterRequest(url));
		...
		public function verify():void {
			...
			var r:URLRequest = twitterRequest(VERIFY_URL);
...
public function loadFavorites(user:String):void{
	var favoritesLoader:URLLoader = this.getLoader(FAVORITES);
	favoritesLoader.load(twitterRequest(FAVORITES_URL.replace("${id}", user)));
}
		
public function createFavorite(status:String):void
{
	var req:URLRequest = twitterRequest(CREATE_FAVORITE_URL.replace("${id}",status));
	req.method = URLRequestMethod.POST;
	this.getLoader(CREATE_FAVORITE).load(req);
}

public function destroyFavorite(status:String):void
{
	var req:URLRequest = twitterRequest(DESTROY_FAVORITE_URL.replace("${id}",status));
	req.method = URLRequestMethod.DELETE;
	this.getLoader(DESTROY_FAVORITE).load(req);
}
...
private function friendCreatedHandler (e:Event) : void{
	//var r:TwitterEvent = new TwitterEvent (TwitterEvent.ON_FAVORITE_CREATED);
	//r.data = "success";
	//dispatchEvent (r);
	//this.getLoader(FOLLOW_USER).data;
}
...
private function favoritesHandler(event:Event):void{
	var xml:XML = new XML(this.getLoader(FAVORITES).data);
	var statusArray:Array = new Array();
    for each (var tempXML:XML in xml.children()) {
		var twitterStatus:TwitterStatus = new TwitterStatus (tempXML)
        statusArray.push(twitterStatus );
    }
	var r:TwitterEvent = new TwitterEvent (TwitterEvent.ON_FAVORITES);
	r.data = statusArray;
	dispatchEvent (r);
}

private function favoriteCreatedHandler(e:Event):void
{
	var r:TwitterEvent = new TwitterEvent (TwitterEvent.ON_FAVORITE_CREATED);
	r.data = "success";
	dispatchEvent (r);
...
private function favoriteDestroyedHandler(e:Event):void
{
	var r:TwitterEvent = new TwitterEvent (TwitterEvent.ON_FAVORITES_DESTROYED);
	r.data = "success";
	dispatchEvent (r);
}
